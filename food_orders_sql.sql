...
--Number of restaurants available on app by cuisine
...
--ORDER BREAKDOWN
--Percent of orders by day (weekday vs weekend)
...
--Percent of order by restaurant

SELECT a1.restaurant_name, COUNT(a1.order_id), COUNT(a1.order_id) * 1.0/(SELECT COUNT(order_id) FROM food_order) Pct_To_Total
FROM food_order a1
...
SELECT a1.cuisine_type, COUNT(a1.order_id), COUNT(a1.order_id) * 1.0/(SELECT COUNT(order_id) FROM food_order) Pct_To_Total
FROM food_order a1
